...
| `--skip-lobs` | | Пропускане на Oracle LOB типове (CLOB, BLOB, NCLOB) | false |
...
### Пропускане на LOB колони

Oracle LOB типовете (CLOB, BLOB, NCLOB) могат да бъдат много големи и може да не са необходими за анализ. Използвайте `--skip-lobs` за да ги изключите:

```bash
# Пропускане на LOB колони за намаляване на размера на файла и подобряване на производителността
oracle2vortex \
  -f query.sql \
  -o data.vortex \
  --host db.example.com \
...
**Как работи:**
- Автоматично открива и филтрира колони съдържащи LOB данни
- LOB се идентифицират по размер (> 4000 символа) или двоични индикатори
- Първият записан запис ще покаже колко колони са пропуснати
- Значително намалява размера на файла и използването на памет за таблици с големи текстови/двоични полета

**Случаи на употреба:**
- Експортиране на таблици с метаданни с полета за описание
- Работа с таблици съдържащи XML или големи JSON документи
- Фокусиране върху структурирани данни при игнориране на двоичното съдържание
- Оптимизация на производителността за таблици с много големи колони
...
- **LOB типове**: По подразбиране LOB колоните (CLOB, BLOB, NCLOB) са включени. Използвайте `--skip-lobs` за да ги изключите за по-добра производителност и по-малки размери на файлове.
